...
var sandwichBarProgress;
var sandwichBar;
var sandwichBarSprite;
...
BlueRoom.Game.prototype.createSandwichView= function () {
    ...
    sandwichBarProgress = 100;
    sandwichBar = this.add.bitmapData(100,10);
    sandwichBarSprite = game.add.sprite(280, 30, sandwichBar);
    sandwichBarSprite.anchor.setTo(0.5, 0.5);

    sandwichViewElements.push(sandwichBarSprite);
...
BlueRoom.Game.prototype.createBar = function(){
    // this.barTimer = 
    setInterval(function(){
    //     if(managerView && !currCust.moving){
    //         
        sandwichBarProgress-=0.1;
    }, 100);
    //     }
...
BlueRoom.Game.prototype.myupdate = function() {
    // ensure you clear the context each time you update it or the bar will draw on top of itself
    sandwichBar.context.clearRect(0, 0, sandwichBar.width, sandwichBar.height);
    
    // some simple colour changing to make it look like a health bar
    if (sandwichBarProgress < 10) {
       sandwichBar.context.fillStyle = '#f00';   
    }
    else if (sandwichBarProgress < 20) {
        sandwichBar.context.fillStyle = '#ff0';
    }
    else {
        sandwichBar.context.fillStyle = '#0f0';
    }

    if(sandwichBarProgress < 0){
        // clearInterval(this.barTimer);
        // clearInterval(this.myTimer);
    }
    
    // draw the bar
    sandwichBar.context.fillRect(0, 0, sandwichBarProgress, 8);
    
    // important - without this line, the context will never be updated on the GPU when using webGL
    sandwichBar.dirty = true;
};
...
BlueRoom.Game.prototype.sandwichUpdate= function () {
    ...
    if(sandwichLine.length==0){
        sandwichBarSprite.visible = false;
    } else{
        sandwichBarSprite.visible = true;
    }
    
    if (sandwichLine.length != 0 && currCustomer == null) {
        console.log("here");
        currCustomer = sandwichLine[0];
        sandwichBarProgress = (currCustomer.happinessBarProgress/30) * 100;
